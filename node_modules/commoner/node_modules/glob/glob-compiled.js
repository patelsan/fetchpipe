...
Glob.prototype._emitMatch = function (index, e) {
  ...
  if (!this.matches[index][e]) {
    ...
    if (!this.stat && !this.mark) return this.emit("match", e);
...
function readdirCb(self, abs, cb) {
  return function (er, entries) {
    if (er) self._readdirError(abs, er, cb);else self._readdirEntries(abs, entries.sort(alphasort), cb);
  };
}
...
Glob.prototype._stat = function (f, cb) {
  var abs = f;
  if (f.charAt(0) === "/") abs = path.join(this.root, f);else if (this.changedCwd) abs = path.resolve(this.cwd, f);
  ...
  if (!this.stat && ownProp(this.cache, f)) {
    ...
    if (abs.slice(-1) === "/" && c !== 'DIR') return cb();
  ...
  if (stat !== undefined) {
    if (stat === false) return cb(null, stat);else return cb(null, stat.isDirectory() ? 'DIR' : 'FILE', stat);
  }

  var self = this;
  var statcb = inflight("stat\0" + abs, statcb_);
  if (statcb) fs.stat(abs, statcb);
