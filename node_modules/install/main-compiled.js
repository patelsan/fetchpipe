...
function Reader(file) {
    ...
    self.addCallback = function (cb) {
        ...
        if (qhead.next === qtail) process.nextTick(flush);
    };
}
...
function rename(installName, code) {
    if (installName !== "install") code = code.replace(/\bglobal\.install\b/g, "global." + installName);
    return code;
}
...
exports.getRequiredIDs = function (id, source) {
    ...
    while (match = requireExp.exec(source)) {
        var rid = match[1];
        if (rid.charAt(0) === ".") rid = path.normalize(path.join(id, "..", match[1]));
