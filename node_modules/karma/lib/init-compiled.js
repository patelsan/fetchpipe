...
var logQueue = [];
var printLogQueue = function printLogQueue() {
...
var installPackage = function installPackage(pkgName) {
...
var validateBrowser = function validateBrowser(name) {
  // TODO(vojta): check if the path resolves to a binary
  installPackage('karma-' + name.toLowerCase().replace('canary', '') + '-launcher');
};

var validateFramework = function validateFramework(name) {
  installPackage('karma-' + name);
};

var validateRequireJs = function validateRequireJs(useRequire) {
  if (useRequire) {
...
var questions = [{
  ...
  options: ['no', 'yes'],
  boolean: true,
  condition: function condition(answers) {
...
var getBasePath = function getBasePath(configFilePath, cwd) {
...
var processAnswers = function processAnswers(answers, basePath, testMainFile) {
