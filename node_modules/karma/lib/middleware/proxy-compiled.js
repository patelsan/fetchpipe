...
var parseProxyConfig = function parseProxyConfig(proxies, config) {
  var endsWithSlash = function endsWithSlash(str) {
    return str.substr(-1) === '/';
  };
  ...
  return _.sortBy(_.map(proxies, function (proxyUrl, proxyPath) {
    ...
    var port = proxyDetails.port || config.port || (proxyDetails.protocol === 'https:' ? '443' : '80');
 ...
 */
var createProxyHandler = function createProxyHandler(proxies, urlRoot) {
  if (!proxies.length) {
