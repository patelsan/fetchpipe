...
var createErrorFormatter = function createErrorFormatter(basePath, emitter, SourceMapConsumer) {
  var lastServedFiles = [];
  ...
  var findFile = function findFile(path) {
  ...
  var URL_REGEXP = new RegExp('(?:https?:\\/\\/[^\\/]*)?\\/?' + '(base|absolute)' + // prefix
  '((?:[A-z]\\:)?[^\\?\\s\\:]*)' + // path
  '(\\?\\w*)?' + // sha
  '(\\:(\\d+))?' + // line
  '(\\:(\\d+))?' + // column
  ...
  return function (msg, indentation) {
    ...
    msg = (msg || '').replace(URL_REGEXP, function (_, prefix, path, __, ___, line, ____, column) {
      ...
      if (file && file.sourceMap) {
        ...
        try {
          var original = getSourceMapConsumer(file.sourceMap).originalPositionFor({ line: line, column: column || 0, bias: bias });

          var formattedColumn = column ? util.format(':%s', column) : '';
          return util.format('%s:%d%s <- %s:%d:%d', path, line, formattedColumn, original.source, original.line, original.column);
        } catch (e) {
...
var createReporters = function createReporters(names, config, emitter, injector) {
