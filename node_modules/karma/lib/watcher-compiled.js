...
var baseDirFromPattern = function baseDirFromPattern(pattern) {
  return pattern.replace(/[\/\\][^\/\\]*\*.*$/, '') // remove parts with *
  .replace(/[\/\\][^\/\\]*[\!\+]\(.*$/, '') // remove parts with !(...) and +(...)
  .replace(/[\/\\][^\/\\]*\)\?.*$/, '') || DIR_SEP; // remove parts with (...)?
};

var watchPatterns = function watchPatterns(patterns, watcher) {
...
var createIgnore = function createIgnore(patterns, excludes) {
...
var onlyWatchedTrue = function onlyWatchedTrue(pattern) {
  return pattern.watched;
};

var getWatchedPatterns = function getWatchedPatterns(patternObjects) {
...
exports.watch = function (patterns, excludes, fileList, usePolling, emitter) {
  ...
  // register events
  chokidarWatcher.on('add', bind(fileList.addFile)).on('change', bind(fileList.changeFile)).on('unlink', bind(fileList.removeFile))
