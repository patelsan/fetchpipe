...
vows.describe('log4js cluster appender').addBatch({
	'when in master mode': {
		topic: function topic() {
			...
			var fakeActualAppender = function fakeActualAppender(loggingEvent) {
		...
		"should register 'fork' event listener on 'cluster'": function shouldRegisterForkEventListenerOnCluster(topic) {
			assert.equal(topic.registeredClusterEvents[0], 'fork');
		},

		"should log using actual appender": function shouldLogUsingActualAppender(topic) {
	...
	'when in worker mode': {
		...
		"worker appender should call process.send": function workerAppenderShouldCallProcessSend(topic) {
		...
		"worker should serialize an Error correctly": function workerShouldSerializeAnErrorCorrectly(topic) {
...
//# sourceMappingURL=clusteredAppender-test-compiled.js.map