...
function makeTestAppender() {
  return {
    configure: function configure(config, options) {
...
vows.describe('log4js configure').addBatch({
  'appenders': {
    'when specified by type': {
      ...
      'should load appender': function shouldLoadAppender(testAppender) {
        assert.ok(testAppender.configureCalled);
      },
      'should pass config to appender': function shouldPassConfigToAppender(testAppender) {
        assert.equal(testAppender.config.flavour, 'gouda');
      },
      'should pass log4js options to appender': function shouldPassLog4jsOptionsToAppender(testAppender) {
    ...
    'when core appender loaded via loadAppender': {
      ...
      'should load appender from ../lib/appenders': function shouldLoadAppenderFromLibAppenders(log4js) {
        assert.ok(log4js.appenders.cheese);
      },
      'should add appender configure function to appenderMakers': function shouldAddAppenderConfigureFunctionToAppenderMakers(log4js) {
    ...
    'when appender in node_modules loaded via loadAppender': {
      ...
      'should load appender via require': function shouldLoadAppenderViaRequire(log4js) {
    ...
    'when appender object loaded via loadAppender': {
      ...
      'should load appender with provided object': function shouldLoadAppenderWithProvidedObject(log4js) {
        assert.ok(log4js.appenders['some/other/external']);
      },
      'should add appender configure function to appenderMakers': function shouldAddAppenderConfigureFunctionToAppenderMakers(log4js) {
        assert.isFunction(log4js.appenderMakers['some/other/external']);
    ...
    'when configuration file loaded via LOG4JS_CONFIG environment variable': {
      topic: function topic() {
        ...
        var fileRead = 0,
          ...
          config: { appenders: [{ type: 'console', layout: { type: 'messagePassThrough' } }],
            levels: { 'a-test': 'INFO' } },
          readdirSync: function readdirSync(dir) {
      ...
      'should load the specified local configuration file': function shouldLoadTheSpecifiedLocalConfigurationFile(fileRead) {
...
//# sourceMappingURL=configuration-test-compiled.js.map