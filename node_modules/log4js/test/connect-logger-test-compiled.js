...
vows.describe('log4js connect logger').addBatch({
  'getConnectLoggerModule': {
    ...
    'should return a "connect logger" factory': function shouldReturnAConnectLoggerFactory(clm) {
      assert.isObject(clm);
    },

    'take a log4js logger and return a "connect logger"': {
      topic: function topic(clm) {
        var ml = new MockLogger();
        var cl = clm.connectLogger(ml);
        return cl;
      },

      'should return a "connect logger"': function shouldReturnAConnectLogger(cl) {
    ...
    'log events': {
      ...
      'check message': function checkMessage(messages) {
    ...
    'logger with options as string': {
      ...
      'should use the passed in format': function shouldUseThePassedInFormat(messages) {
    ...
    'auto log levels': {
      ...
      'should use INFO for 2xx': function shouldUseINFOFor2xx(messages) {
        assert.ok(levels.INFO.isEqualTo(messages[0].level));
        assert.ok(levels.INFO.isEqualTo(messages[1].level));
      },

      'should use WARN for 3xx': function shouldUseWARNFor3xx(messages) {
        assert.ok(levels.WARN.isEqualTo(messages[2].level));
      },

      'should use ERROR for 4xx': function shouldUseERRORFor4xx(messages) {
        assert.ok(levels.ERROR.isEqualTo(messages[3].level));
      },

      'should use ERROR for 5xx': function shouldUseERRORFor5xx(messages) {
    ...
    'format using a function': {
      ...
      'should call the format function': function shouldCallTheFormatFunction(messages) {
    ...
    'format that includes request headers': {
      ...
      'should output the request header': function shouldOutputTheRequestHeader(messages) {
    ...
    'format that includes response headers': {
      ...
      'should output the response header': function shouldOutputTheResponseHeader(messages) {
...
//# sourceMappingURL=connect-logger-test-compiled.js.map