...
vows.describe('../lib/appenders/dateFile').addBatch({
  'appender': {
    'adding multiple dateFileAppenders': {
      ...
      'should only add one `exit` listener': function shouldOnlyAddOneExitListener(initialCount) {
    ...
    'exit listener': {
      topic: function topic() {
        var exitListener,
          ...
          requires: {
            '../streams': {
              DateRollingFileStream: function DateRollingFileStream(filename) {
    ...
    'with default settings': {
      ...
      'should use the basic layout': function shouldUseTheBasicLayout(contents) {
  ...
  'configure': {
    ...
    'with options.alwaysIncludePattern': {
      ...
      'should create file with the correct pattern': function shouldCreateFileWithTheCorrectPattern(contents) {
        assert.include(contents, 'this should be written to the file with the appended date');
      },
      'should not overwrite the file on open (bug found in issue #132)': function shouldNotOverwriteTheFileOnOpenBugFoundInIssue132(contents) {
    ...
    'with cwd option': {
      topic: function topic() {
        var fileOpened,
            appender = sandbox.require('../lib/appenders/dateFile', { requires: { '../streams': { DateRollingFileStream: function DateRollingFileStream(file) {
                fileOpened = file;
                return {
                  on: function on() {},
                  end: function end() {}
...
//# sourceMappingURL=dateFileAppender-test-compiled.js.map