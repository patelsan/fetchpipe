...
vows.describe('log4js fileAppender').addBatch({
  'adding multiple fileAppenders': {
    ...
    'does not add more than one `exit` listeners': function doesNotAddMoreThanOneExitListeners(initialCount) {
  ...
  'exit listener': {
    topic: function topic() {
      var exitListener,
        ...
        requires: {
          '../streams': {
            RollingFileStream: function RollingFileStream(filename) {
    ...
    'should close all open files': function shouldCloseAllOpenFiles(openedFiles) {
  ...
  'with default fileAppender settings': {
    ...
    'should write log messages to the file': function shouldWriteLogMessagesToTheFile(err, fileContents) {
      assert.include(fileContents, "This should be in the file." + EOL);
    },
    'log messages should be in the basic layout format': function logMessagesShouldBeInTheBasicLayoutFormat(err, fileContents) {
  ...
  'fileAppender subcategories': {
    ...
    'check file contents': function checkFileContents(err, fileContents) {
  ...
  'with a max file size and no backups': {
    ...
    'log file should only contain the second message': function logFileShouldOnlyContainTheSecondMessage(err, fileContents) {
    ...
    'the number of files': {
      ...
      'starting with the test file name should be two': function startingWithTheTestFileNameShouldBeTwo(err, files) {
  ...
  'with a max file size and 2 backups': {
    ...
    'the log files': {
      topic: function topic(files) {
        var logFiles = files.filter(function (file) {
          return file.indexOf('fa-maxFileSize-with-backups-test.log') > -1;
        });
        return logFiles;
      },
      'should be 3': function shouldBe3(files) {
        assert.equal(files.length, 3);
      },
      'should be named in sequence': function shouldBeNamedInSequence(files) {
      ...
      'and the contents of the first file': {
        topic: function topic(logFiles) {
          fs.readFile(path.join(__dirname, logFiles[0]), "utf8", this.callback);
        },
        'should be the last log message': function shouldBeTheLastLogMessage(contents) {
      ...
      'and the contents of the second file': {
        ...
        'should be the third log message': function shouldBeTheThirdLogMessage(contents) {
      ...
      'and the contents of the third file': {
        ...
        'should be the second log message': function shouldBeTheSecondLogMessage(contents) {
  ...
  'configure': {
    'with fileAppender': {
      ...
      'should load appender configuration from a json file': function shouldLoadAppenderConfigurationFromAJsonFile(err, contents) {
  ...
  'when underlying stream errors': {
    ...
    'should log the error to console.error': function shouldLogTheErrorToConsoleError(consoleArgs) {
...
//# sourceMappingURL=fileAppender-test-compiled.js.map