...
vows.describe('log4js logLevelFilter').addBatch({
  'appender': {
    ...
    'should only pass log events greater than or equal to its own level': function shouldOnlyPassLogEventsGreaterThanOrEqualToItsOwnLevel(logEvents) {
  ...
  'configure': {
    ...
    'tmp-tests.log': {
      ...
      'should contain all log messages': function shouldContainAllLogMessages(contents) {
    ...
    'tmp-tests-warnings.log': {
      ...
      'should contain only error and warning log messages': function shouldContainOnlyErrorAndWarningLogMessages(contents) {
        var messages = contents.trim().split(EOL);
    ...
    'tmp-tests-debugs.log': {
      ...
      'should contain only trace and debug log messages': function shouldContainOnlyTraceAndDebugLogMessages(contents) {
        var messages = contents.trim().split(EOL);
...
//# sourceMappingURL=logLevelFilter-test-compiled.js.map