...
vows.describe('../lib/logger').addBatch({
  'constructor with no parameters': {
    topic: new Logger(),
    'should use default category': function shouldUseDefaultCategory(logger) {
      assert.equal(logger.category, Logger.DEFAULT_CATEGORY);
    },
    'should use TRACE log level': function shouldUseTRACELogLevel(logger) {
  ...
  'constructor with category': {
    ...
    'should use category': function shouldUseCategory(logger) {
  ...
  'constructor with category and level': {
    ...
    'should use level': function shouldUseLevel(logger) {
  ...
  'isLevelEnabled': {
    ...
    'should provide a level enabled function for all levels': function shouldProvideALevelEnabledFunctionForAllLevels(logger) {
    ...
    'should return the right values': function shouldReturnTheRightValues(logger) {
  ...
  'should emit log events': {
    ...
    'when log writes are enabled': function whenLogWritesAreEnabled(events) {
      assert.equal(events[0].data[0], 'Event 1');
    },

    'but not when log writes are disabled': function butNotWhenLogWritesAreDisabled(events) {
...
//# sourceMappingURL=logger-test-compiled.js.map