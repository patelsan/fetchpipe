...
vows.describe('log4js').addBatch({
  ...
  'getBufferedLogger': {
    ...
    'should take a category and return a logger': function shouldTakeACategoryAndReturnALogger(logger) {
    ...
    'cache events': {
      ...
      'should not emit log events if .flush() is not called.': function shouldNotEmitLogEventsIfFlushIsNotCalled(events) {
    ...
    'log events after flush() is called': {
      ...
      'should emit log events when .flush() is called.': function shouldEmitLogEventsWhenFlushIsCalled(events) {
  ...
  'getLogger': {
    ...
    'log events': {
      ...
      'should emit log events': function shouldEmitLogEvents(events) {
      ...
      'should not emit events of a lower level': function shouldNotEmitEventsOfALowerLevel(events) {
        assert.equal(events.length, 4);
        assert.equal(events[1].level.toString(), 'WARN');
      },

      'should include the error if passed in': function shouldIncludeTheErrorIfPassedIn(events) {
  ...
  'when shutdown is called': {
    topic: function topic() {
      var events = {
        ...
        requires: {
          ...
          shutdownCallback = function shutdownCallback() {
        events.shutdownCallbackCalled = true;
      },
          config = { appenders: [{ "type": "file",
    ...
    'should invoke appender shutdowns': function shouldInvokeAppenderShutdowns(events) {
      assert.ok(events.appenderShutdownCalled);
    },

    'should call callback': function shouldCallCallback(events) {
      assert.ok(events.shutdownCallbackCalled);
    }
  },

  'invalid configuration': {
    'should throw an exception': function shouldThrowAnException() {
  ...
  'configuration when passed as object': {
    ...
    'should be passed to appender config': function shouldBePassedToAppenderConfig(configuration) {
  ...
  'configuration that causes an error': {
    ...
    'should wrap error in a meaningful message': function shouldWrapErrorInAMeaningfulMessage(e) {
  ...
  'configuration when passed as filename': {
    topic: function topic() {
      var appenderConfig,
          configFilename,
          log4js = sandbox.require('../lib/log4js', { requires: { 'fs': { statSync: function statSync() {
    ...
    'should read the config from a file': function shouldReadTheConfigFromAFile(args) {
      assert.equal(args[0], '/path/to/cheese.json');
    },
    'should pass config to appender': function shouldPassConfigToAppender(args) {
  ...
  'with no appenders defined': {
    ...
    'should default to the console appender': function shouldDefaultToTheConsoleAppender(evt) {
  ...
  'addAppender': {
    ...
    'without a category': {
      'should register the function as a listener for all loggers': function shouldRegisterTheFunctionAsAListenerForAllLoggers(log4js) {
        var appenderEvent,
            appender = function appender(evt) {
          appenderEvent = evt;
        },
      ...
      'if an appender for a category is defined': {
        'should register for that category': function shouldRegisterForThatCategory(log4js) {
    ...
    'with a category': {
      'should only register the function as a listener for that category': function shouldOnlyRegisterTheFunctionAsAListenerForThatCategory(log4js) {
        var appenderEvent,
            appender = function appender(evt) {
          appenderEvent = evt;
        },
    ...
    'with multiple categories': {
      'should register the function as a listener for all the categories': function shouldRegisterTheFunctionAsAListenerForAllTheCategories(log4js) {
        var appenderEvent,
            appender = function appender(evt) {
          appenderEvent = evt;
        },
      ...
      'should register the function when the list of categories is an array': function shouldRegisterTheFunctionWhenTheListOfCategoriesIsAnArray(log4js) {
        var appenderEvent,
            appender = function appender(evt) {
          appenderEvent = evt;
        };
  ...
  'default setup': {
    ...
    'should configure a console appender': function shouldConfigureAConsoleAppender(appenderEvents) {
      assert.equal(appenderEvents[0].data[0], 'this is a test');
    },

    'should not replace console.log with log4js version': function shouldNotReplaceConsoleLogWithLog4jsVersion(appenderEvents) {
  ...
  'console': {
    ...
    'when replaceConsole called': {
      ...
      'should replace console.log methods with log4js ones': function shouldReplaceConsoleLogMethodsWithLog4jsOnes(logEvents) {
    ...
    'when turned off': {
      ...
      'should call the original console methods': function shouldCallTheOriginalConsoleMethods(err) {
  ...
  'console configuration': {
    ...
    'when disabled': {
      ...
      'should allow for turning off console replacement': function shouldAllowForTurningOffConsoleReplacement(err) {
        assert.instanceOf(err, Error);
        assert.equal(err.message, 'this should not be called.');
      }
    },
    'when enabled': {
      ...
      'should allow for turning on console replacement': function shouldAllowForTurningOnConsoleReplacement(logEvents) {
  ...
  'configuration persistence': {
    ...
    'should maintain appenders between requires': function shouldMaintainAppendersBetweenRequires(logEvent) {
  ...
  'getDefaultLogger': {
    ...
    'should return a logger': function shouldReturnALogger(logger) {
...
//# sourceMappingURL=logging-test-compiled.js.map