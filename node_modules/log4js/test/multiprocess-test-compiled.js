...
function makeFakeNet() {
  return {
    ...
    fakeAppender: function fakeAppender(logEvent) {
      this.logEvents.push(logEvent);
    },
    createConnection: function createConnection(port, host) {
      var fakeNet = this;
      this.port = port;
      this.host = host;
...
vows.describe('Multiprocess Appender').addBatch({
  'worker': {
    topic: function topic() {
      var fakeNet = makeFakeNet(),
          appender = sandbox.require('../lib/appenders/multiprocess', {
        requires: {
          'net': fakeNet
        }
    ...
    'should open a socket to the loggerPort and loggerHost': function shouldOpenASocketToTheLoggerPortAndLoggerHost(net) {
      assert.equal(net.port, 1234);
      assert.equal(net.host, 'pants');
    },
    'should buffer messages written before socket is connected': function shouldBufferMessagesWrittenBeforeSocketIsConnected(net) {
      assert.equal(net.data[0], JSON.stringify('before connect'));
    },
    'should write log messages to socket as json strings with a terminator string': function shouldWriteLogMessagesToSocketAsJsonStringsWithATerminatorString(net) {
      assert.equal(net.data[0], JSON.stringify('before connect'));
    ...
    'should attempt to re-open the socket on error': function shouldAttemptToReOpenTheSocketOnError(net) {
    ...
    'should serialize an Error correctly': function shouldSerializeAnErrorCorrectly(net) {
  ...
  'worker with timeout': {
    ...
    'should attempt to re-open the socket': function shouldAttemptToReOpenTheSocket(net) {
  ...
  'worker defaults': {
    ...
    'should open a socket to localhost:5000': function shouldOpenASocketToLocalhost5000(net) {
  ...
  'master': {
    ...
    'should listen for log messages on loggerPort and loggerHost': function shouldListenForLogMessagesOnLoggerPortAndLoggerHost(net) {
      assert.equal(net.port, 1234);
      assert.equal(net.host, 'server');
    },
    'should return the underlying appender': function shouldReturnTheUnderlyingAppender(net) {
      assert.equal(net.logEvents[0], 'this should be sent to the actual appender directly');
    },
    'when a client connects': {
      topic: function topic(net) {
        var logString = JSON.stringify({ level: { level: 10000, levelStr: 'DEBUG' },
          data: ['some debug'] }) + '__LOG4JS__';

        net.cbs.data(JSON.stringify({ level: { level: 40000, levelStr: 'ERROR' },
          data: ['an error message'] }) + '__LOG4JS__');
        net.cbs.data(logString.substring(0, 10));
        net.cbs.data(logString.substring(10));
        net.cbs.data(logString + logString + logString);
        net.cbs.end(JSON.stringify({ level: { level: 50000, levelStr: 'FATAL' },
          data: ["that's all folks"] }) + '__LOG4JS__');
        net.cbs.data('bad message__LOG4JS__');
        return net;
      },
      'should parse log messages into log events and send to appender': function shouldParseLogMessagesIntoLogEventsAndSendToAppender(net) {
      ...
      'should parse log messages split into multiple chunks': function shouldParseLogMessagesSplitIntoMultipleChunks(net) {
      ...
      'should parse multiple log messages in a single chunk': function shouldParseMultipleLogMessagesInASingleChunk(net) {
      ...
      'should handle log messages sent as part of end event': function shouldHandleLogMessagesSentAsPartOfEndEvent(net) {
        assert.equal(net.logEvents[6].data[0], "that's all folks");
      },
      'should handle unparseable log messages': function shouldHandleUnparseableLogMessages(net) {
  ...
  'master defaults': {
    ...
    'should listen for log messages on localhost:5000': function shouldListenForLogMessagesOnLocalhost5000(net) {
      assert.equal(net.port, 5000);
      assert.equal(net.host, 'localhost');
    }
  }
  ...
  'configure': {
    topic: function topic() {
      var results = {},
        ...
        requires: {
          'net': fakeNet,
          '../log4js': {
            loadAppender: function loadAppender(app) {
              results.appenderLoaded = app;
            },
            appenderMakers: {
              'madeupappender': function madeupappender(config, options) {
    ...
    'should load underlying appender for master': function shouldLoadUnderlyingAppenderForMaster(results) {
      assert.equal(results.appenderLoaded, 'madeupappender');
    },
    'should pass config to underlying appender': function shouldPassConfigToUnderlyingAppender(results) {
      assert.equal(results.config.cheese, 'gouda');
    },
    'should pass options to underlying appender': function shouldPassOptionsToUnderlyingAppender(results) {
      assert.equal(results.options.crackers, 'jacobs');
    }
  }
}).exportTo(module);

//# sourceMappingURL=multiprocess-test-compiled.js.map