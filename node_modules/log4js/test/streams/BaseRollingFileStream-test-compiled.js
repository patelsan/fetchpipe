...
vows.describe('../../lib/streams/BaseRollingFileStream').addBatch({
  'when node version < 0.10.0': {
    ...
    'it should use readable-stream to maintain compatibility': function itShouldUseReadableStreamToMaintainCompatibility(required) {
  ...
  'when node version > 0.10.0': {
    ...
    'it should use the core stream module': function itShouldUseTheCoreStreamModule(required) {
      assert.ok(required.stream);
      assert.ok(!required['readable-stream']);
    }
  },
  ...
  'when no filename is passed': {
    ...
    'it should throw an error': function itShouldThrowAnError(BaseRollingFileStream) {
      try {
  ...
  'default behaviour': {
    ...
    'it should not want to roll': function itShouldNotWantToRoll(stream) {
      assert.isFalse(stream.shouldRoll());
    },
    'it should not roll': function itShouldNotRoll(stream) {
      var cbCalled = false;
...
//# sourceMappingURL=BaseRollingFileStream-test-compiled.js.map