...
function which(args, extraPath, cb) {
  if (typeof extraPath === 'function') cb = extraPath, extraPath = null;
  ...
  if (extraPath) {
    ...
    options.env = Object.keys(process.env).reduce(function (env, k) {
      if (!k.match(/^path$/i)) env[k] = process.env[k];
      return env;
    }, { PATH: p });
  }
  ...
  cases.forEach(function (c) {
    t.test(c, function (t) {
      which(['which', c], p, function (code, signal, out, err) {
        ...
        t.equal(code, 0);
        t.equal(err, '');
        if (/s/.test(c)) t.equal(out, '', 'should be silent');else if (/a/.test(c)) t.ok(out.split(/\n/).length > 1, 'should have more than 1 result');
        t.end();
      });
    });
  });
